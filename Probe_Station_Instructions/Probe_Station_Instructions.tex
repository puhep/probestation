...
\begin{center}
\bf \LARGE
P3MD LAB -\\PROBE STATION INSTRUCTIONS v2.3
\end{center}
\vspace{5mm}

The semi-automatic probe station, Cascade Summit 12000 series, is located in the P3MD Lab in PHYS-360. This document describes the various parts of  the probe station, and provides instructions on using the probe station. Please follow the instructions below in order to use the probe station.
...
There are many components that make up the probe station unit. The unit consists of:
\begin{enumerate}
\begin{enumerate}
...
\vspace{2mm}
{\color{red}
IF YOU SEE ANY INSTRUMENT NOT WORKING OR NOTICE ANYTHING UNUSUAL, PLEASE INFORM A SENIOR MEMBER IN THE LAB IMMEDIATELY!}
\vspace{2mm}

The probe station is used for measuring IV (current vs. voltage) and CV (capacitance vs. voltage) curves for different pad positions on a wafer. The K6485 are picoammeters used for measuring currents from pads on the wafer. The Keithley 237 Source meter is used to supply bias to the wafer, sometimes by biasing the chuck. The LCR meter is used for low frequency as well as high frequency CV measurements (limit is -500V). The switching matrix is used to switch between IV and CV measurements directly, without changing the setup (not yet implemented in the current setup). The Temptronic thermal system is used to maintain the wafer at a given temperature.

Figure 2 shows the various instruments used for focusing and holding the wafer. The A-Zoom box provides the light for viewing the wafer during setup and can be switched off manually to power off the light when measurements are being made. A-Zoom also provides focusing using two meter dials as shown. Figure 2 also shows the joystick which can be used for moving the view to various positions on the wafer. The vacuum wafer holder (vacuum pen) is also shown in Figure 2. The vacuum holder only works when the switch on its side is pressed down. Letting go of the switch turns off the vacuum and wafer will fall down, SO PLEASE MAKE SURE TO KEEP PRESSING THE SWITCH WHILE USING THE VACUUM HOLDER. 
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.85\textwidth]{instruments.png}
...
Figure 3 also shows the four probe needles. There are controls for moving the probe needles left-right, up-down and, most importantly, lower-higher. The left-right and up-down knobs move the probe needle in XY plane. The lower-higher (Z direction) moves the probe needles towards or away from the wafer. Since two cameras are used to capture the image of the wafer, when a needle is further away from the wafer it appears as 2 needles, while when the needle is closer to wafer it appears as a single needle. All the needle control knobs are very sensitive and should be used slowly and carefully. 


%Section 3 - Probes and cabling
\section{Probes and cabling}
The output of K237 provides the bias to the chuck, which goes to the probe station. Also, the outputs from probes are connected to Pins 1 and 2 on the top of the probe station. These outputs are connected by BNC connectors to the Keithley 6485 picoammeters. All the COM terminals of 6485 and 237 are connected together to form a common ground. All units are also connected to a PC using GPIB cables. These different GPIB cables are daisy-chained together and all the bias voltages and current measurements can be displayed and recorded on the computer using GPIB connections. LabView code is used to measure both IV and CV data and automatically apply bias using the display panel on the computer.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.95\textwidth]{outputs1.png}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.95\textwidth]{outputs2.png}
...
Open Nucleus by double-clicking the Nucleus icon \includegraphics[width=.05\textwidth]{nucleus_icon.png} on the desktop. The login credentials are as follows:
...
\begin{enumerate}
\begin{enumerate}
	\item First, click the {\it ``Wafer Load''} button \includegraphics[width=.05\textwidth]{wafer_load.png} in the motion control sub-window. The Live Video window will show the view's position moving from the center of the chuck to the top edge of the chuck.
	\item Next, open the probe station micro-chamber door. A message will be displayed saying that the wafer access interlock is open. The chuck and chuck interlock should be visible as shown in Figure 6. Unlatch the chuck interlock and pull out the chuck. Pull the chuck all the way back until you feel a click. The chuck should now be held in place and will not slide back in after you let go.
	\item Check to make sure that the {\it ``Vacuum Off''} button \includegraphics[width=.05\textwidth]{vacuum_off.png} in the main panel is set, indicating that the chuck's vacuum is off. Otherwise, the vacuum will be ON before you place the wafer down.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.42\textwidth]{chuck1.png}
	\includegraphics[width=0.41\textwidth]{chuck2.png}
...
\begin{enumerate}
\begin{enumerate}
	\item Check if the lever arm on the left side of the probe station is down. If not, lower the lever arm (to lower the platen).
	\item Make sure the vacuum pump is turned on.
	\item If using conductive rubber, place it on the chuck and align it so that the vacuum holes on the chuck can be seen through the holes in the conductive rubber.
	...
	{\color{red}Make sure that the vacuum holder black button is always PRESSED ON when the wafer is held; otherwise the wafer will fall.}
	...
	{\color{red}Once the wafer is released from the vacuum pen it may slide around.}
	...
	\item Press the {\it ``Vacuum On''} button in the main panel (same location as the {\it ``Vacuum Off"} button, should look like this when the vacuum is on: \includegraphics[width=.05\textwidth]{vacuum_on.png}). Check with the tweezer or vacuum pen that the wafer is stuck to chuck and does not move.
	\item Align the wafer as much as possible using an eyeball estimate. To do this, use the latch to push the wafer towards the micro-chamber so that edge of micro-chamber door is parallel to any of the major lines on the wafer. If the wafer is not aligned, turn off the vacuum and very slowly move the wafer to align it to edge of the chamber. Alternatively, use the theta-knob underneath the chuck to rotate the chuck and align the wafer. Try to align it as much as possible. Fine alignment of the wafer will be done using software.
	...
	\item Turn the light ON by flipping the A-Zoom power switch, turning the light knobs on the A-Zoom focus controller, and pressing the auxiliary power on button in Nucleus.
	\item In Nucleus click the {\it ``Wafer Center''} button \includegraphics[width=.05\textwidth]{wafer_center.png} in the motion control window.
\end{enumerate}
\end{enumerate}
...
\begin{enumerate}
\begin{enumerate}
	\item Before moving to contact, check the contact settings. Right-click the {\it ``Move Z Stage to Contact''} button \includegraphics[width=.05\textwidth]{z_stage_contact_off.png} and ensure that the settings under ``Chuck'' are as follows:
	\begin{enumerate}
		\item {\it Contact}: 1000 um
		\item {\it Separate Distance}: 500 um
	\end{enumerate}
	\item Make sure ALL of the probes are a safe Z-distance above the wafer, then click on the {\it ``Move Z Stage to Contact''} button \includegraphics[width=.05\textwidth]{z_stage_contact_off.png}. The symbol should turn green \includegraphics[width=.05\textwidth]{z_stage_contact_on.png}, indicating that the probe station is in contact mode. While in this mode you cannot move the chuck using the Nucleus software.
	...
	\item {\color{red}Turn the light off.} This is best done by pressing the auxiliary power off button in Nucleus.
\end{enumerate}
\end{enumerate}
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\vspace{1mm}
		\\{\color{red}NOTE: If the K237 is not in Standby mode when the light is switched on you WILL arc the sensor and possibly ruin the entire wafer. So make sure that the light is only turned on when the K237 is in standby.}
		\item Check to see if the probes are still aligned with their respective pads. If they are, raise and then very gently lower the probe tips until they are in contact with the pads.
		\item Another possibility for the contact check is that the probe station did not go back into contact mode after moving to the site. To check this, look at the status of the Stage Height in the Motion Control window. If it is set to {\it ``Separate''} \includegraphics[width=.05\textwidth]{z_stage_separate_on.png} check to make sure the probes are aligned above their respective pads and then click {\it ``Move Z Stage to Contact''} \includegraphics[width=.05\textwidth]{z_stage_contact_off.png}.
		\item {\color{red}Turn the light off.}
...
\begin{enumerate}
	...
	\item Click the {\it ``Move the Z Stage to separate''} button \includegraphics[width=.05\textwidth]{z_stage_separate_off.png} in the Motion Control window. It should turn red to indicate that the probe station is in separate mode \includegraphics[width=.05\textwidth]{z_stage_separate_on.png}.
	\item For additional separation, raise the lever arm on the side of the probe station to raise the platen.
	\item Click the {\it ``Wafer Load''} button \includegraphics[width=.05\textwidth]{wafer_load.png} the Nucleus Motion Control window. This will bring the chuck forward so that it can be pulled out and accessed easily.
	\item Open the micro-chamber gate. Pull out the chuck and secure it in place.
	\item Click the {\it ``Turn Vacuum OFF''} symbol (should look like this when the vacuum is turned off: \includegraphics[width=.05\textwidth]{vacuum_off.png}) on the Nucleus window.
